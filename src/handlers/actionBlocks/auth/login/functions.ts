...
import {ActionResult} from "@lib/action/createAction";

const checkSafemode = async (userDoc, fingerPrint): Promise<ActionResult> => {
...
export const checkCredentials = async (stdID, password, fingerPrint, userCollection, verify): Promise<ActionResult> => {
  ...
  if (verify !== "") {
    const task = await initialiseDB.collection("tasks").doc(verify).get()
    if (task.exists) {
  ...
  if (userDoc.get("safeMode") === true && !verified) {
    const chekSafeModeResult = await checkSafemode(userDoc, fingerPrint)
    if (!chekSafeModeResult.status) return chekSafeModeResult
  }
  ...
  return {status: true, report: "success", data: userDoc}
}
...
export const appendSession = async (sessionsColl, userDoc, fingerPrint ,req, res) => {
