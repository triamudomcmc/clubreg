...
export const checkExistedBrowser = async (userData, fingerPrint) => {
  let match = false

  if ("authorised" in userData) {
    const authorisedObj: LooseTypeObject<{fingerPrint: string}> = userData["authorised"]
    match = Object.values(authorisedObj).some(val => (val.fingerPrint === fingerPrint))
  }
...
export const appendDeviceToDB = async (fingerPrint, ua, userID, clientIp) => {
  const randomDeviceID = cryptoRandomString({length: 20})

  await initialisedDB.collection("users").doc(userID).set({
