...
const Account = () => {
  ...
  const [clubData, setClubData] = useState({new_count: 0, new_count_limit: 0, place: "", audition: false, message: "", contact: {type: "", context: ""}, contact2: {type: "", context: ""}, contact3: {type: "", context: ""}})
        ...
        <div className="pt-8 pb-20 px-4 max-w-6xl mx-auto">
          ...
          <div className="flex flex-col mt-20 space-y-14 px-2 md:px-4">
            <div>
              ...
              <div className="border-b border-gray-200 py-4 md:py-6 space-y-1 md:flex md:items-center md:space-y-0 md:space-x-[183px]">
                <h1 className="text-TUCMC-gray-500">ข้อความถึงสมาชิกชมรม</h1>
                <h1>{clubData.message}</h1>
              </div>
              <div className="border-b border-gray-200 py-4 md:py-6 space-y-1 md:flex md:items-center md:space-y-0 md:space-x-[184px]">
                <h1 className="text-TUCMC-gray-500">ช่องทางการติดต่อชมรม</h1>
                <div>
                  <p className={classnames(isEmpty(clubData.contact) ? "hidden" : "block")}>{clubData.contact?.type} : {clubData.contact?.context}</p>
                  <p className={classnames(isEmpty(clubData.contact2) ? "hidden" : "block")}>{clubData.contact2?.type} : {clubData.contact2?.context}</p>
                  <p className={classnames(isEmpty(clubData.contact3) ? "hidden" : "block")}>{clubData.contact3?.type} : {clubData.contact3?.context}</p>
                </div>
              </div>
              <div className="border-b border-gray-200 py-4 md:py-6 space-y-1 md:flex md:items-center md:space-y-0 md:space-x-[165px]">
