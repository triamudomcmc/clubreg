...
import { fetchClubCommitteeAction } from "./mainFunction"

export const fetchClubCommittee = async (req, res) => {
  // if (new Date().getTime() >= editDataTime) return { status: false, report: "exceeded_time_limit" }

  return await executeWithPermission(req, res, fetchClubCommitteeAction)
}
