...
export const regClub = async (req, res) => {
  ...
  if (new Date().getTime() < lastround || new Date().getTime() >= endLastRound) return {status: false, report: "exceeded_time_limit"}
  ...
  try {
    ...
    if (clubData.audition && !req.body.oldClubConfirm) {
      ...
      if (Object.keys(currentData.get("audition") || {}).length <= 0) { return {status: false, report: "denied"}}
