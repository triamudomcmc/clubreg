...
export const getUserCredentialsFromID = async (userID, req) => {
  ...
  const authorisedArr = rawData ? Object.keys(rawData).map(key => {
    return {...{id: key, browser: rawData[key].browser, os: rawData[key].os, device: rawData[key].device, cpu: rawData[key].cpu, ip: rawData[key].ip}, self: req.body.fp === rawData[key].fingerPrint}
  }): []
