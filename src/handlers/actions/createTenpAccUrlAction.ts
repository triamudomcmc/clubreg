...
export const createTempAccUrlAction = createTempAccUrlBridge.helper.createAction(
  async (ApiParams, parameters, ConditionParams) => {
    ...
    return {
      ...
      report: "success",
      data: {
        accessUrl: `https://register.clubs.triamudom.ac.th/panel/attendance?access=${taskDoc.id}&route=${parameters.club}&targetTime=${parameters.timestamp}&expire=${expire}`,
      },
    }
  }
)
